        ...
        <section className="space-y-4">
            ...
            <div ref={scrollContainerRef} className="flex gap-4 overflow-x-auto scrollbar-hide pb-4 -mx-4 px-4">
                {items.map((item, index) => {
                    ...
                    const animeId = (item as any).mal_id || item.anime_id;
                    const name = item.name;
                    const score = isRecommendation ? (item as any).predicted_rating : (item as any).score || (item as any).rating;
                    const genres = (item as any).genres || (item as any).genre || [];
                    const episodes = !isRecommendation ? (item as any).episodes : undefined;
                    const type = !isRecommendation ? (item as any).type : undefined;
